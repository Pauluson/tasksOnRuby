...
class Menu
  ...
  def create_route
    ...
    routes << Route.new(stations[first_station - 1], stations[finish_station - 1])
  end
  ...
  def carriges
    ...
    if choice == 1
      ...
      show_trains
      number_of_train = gets.chomp.to_i
      if trains[number_of_train - 1].type == 'Cargo'
        trains[number_of_train - 1].add_carriage(CargoCarriage.new)
      elsif trains[number_of_train - 1].type == 'Passenger'
        trains[number_of_train - 1].add_carriage(PassengerCarriage.new)
      end
    elsif choice == 2
      puts 'On what train do you want to delete carriage?'
      show_trains
      number_of_train = gets.chomp.to_i
      trains[number_of_train - 1].delete_carriage(carriage)
    end
  end
  ...
  def fixed_route
    ...
    if number_of_choice == 1
      ...
      routes[number_of_route - 1].add_station(stations[number_of_station - 1])
    elsif number_of_choice == 2
      puts 'Input station to delete'
      routes[number_of_route - 1].get_all_stations
      station_to_delete = gets.chomp.to_i
      routes[number_of_route - 1].delete_station(stations[station_to_delete - 1])
    end
  end
  ...
  def move_trains
    ...
    choice = gets.chomp.to_i
    if choice == 1
      trains[number_of_train - 1].move_next_station
    elsif choice == 2
      trains[number_of_train - 1].move_previous_station
    end
  end
